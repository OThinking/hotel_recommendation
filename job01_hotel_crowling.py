...
for idx, url in enumerate(review_url):
    ...
    if (idx+1) % 30 == 0:
        ...
        #     df.to_csv('./crawling_data/reviews_gyeonggi{}.csv'.format(idx), index=False)

# df = pd.DataFrame({'names':hap_hotel_name, 'reviews':reviews})
# df.to_csv('./crawling_data/reviews_gyeonggi.csv', index=False)